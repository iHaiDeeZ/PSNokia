...
address _my_last_native_call = 0;

void my_trace_native_entry() {
  /* Full per-call logging throttles real execution to a few FPS (see
   * git history) - but _my_last_native_call itself is already updated
   * on every native call for free by the ARM codegen (a single store,
   * done before this function is even entered). Sampling it at a fixed
   * low rate costs one file-I/O every ~300ms, not one per call, and is
   * enough to tell whether the VM is spinning on a specific native
   * call or genuinely blocked (never calling anything). */
#if __has_include(<psp2/io/fcntl.h>)
  static SceUInt64 lastSample = 0;
  SceUInt64 now = sceKernelGetProcessTimeWide();
  if (now - lastSample >= 300000) { /* microseconds */
    char buf[64];
    int len = jvm_sprintf(buf, "SAMPLE native=%p\n", (void*)_my_last_native_call);
