...
int platformRequest(char* pszUrl) {
    ...
    /* PLATFORM_REQUEST_KEY is never configured for vita_arm (only the
     * linux_qte port's properties.xml sets it), so this branch is dead
     * in practice - but if it's ever configured in the future, honestly
     * report failure rather than claiming success while doing nothing:
     * no process is actually spawned below. */
